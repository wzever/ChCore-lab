...
s32 signal_sem(struct semaphore *sem)
{
        ...
        struct thread *awake = NULL;
        if (sem->sem_count > 0 || sem->waiting_threads_count == 0){
                sem->sem_count++;
        } else{
                awake = list_entry(sem->waiting_threads.next, struct thread, sem_queue_node);
                list_del(&awake->sem_queue_node);
                --sem->waiting_threads_count;
        }
        
        if (awake) {
                BUG_ON(awake->thread_ctx->sc == NULL);
                awake->thread_ctx->state = TS_INTER;
                BUG_ON(sched_enqueue(awake));
        }
        /* LAB 4 TODO END */
        return 0;
}
